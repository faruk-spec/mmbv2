...
class MailService
{
    ...
    public static function ensureSchema(): void
    {
        ...
        try {
            ...
            // 9. Upgrade support-ticket templates with improved design (idempotent: detect by {{app_name}} absence)
            $stCreated = $db->fetch(
                "SELECT id, body FROM mail_notification_templates WHERE slug = 'support-ticket-created' LIMIT 1",
                []
            );
            if ($stCreated && strpos($stCreated['body'] ?? '', '{{app_name}}') === false) {
                $bodyCreated = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="margin:0;padding:0;background:#f0f4f8;font-family:\'Segoe UI\',Arial,sans-serif;"><table role="presentation" style="width:100%;border-collapse:collapse;"><tr><td style="padding:32px 16px;"><table role="presentation" style="max-width:600px;margin:0 auto;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.10);"><tr><td style="background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%);padding:36px 40px 28px;text-align:center;"><div style="display:inline-block;width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,#00c6ff,#a855f7);text-align:center;line-height:52px;font-size:24px;margin-bottom:14px;">🎫</div><h1 style="margin:0;color:#fff;font-size:22px;font-weight:700;letter-spacing:-.3px;">Ticket #{{ticket_id}} Created</h1><p style="margin:8px 0 0;color:rgba(255,255,255,.65);font-size:13px;">{{app_name}} Support</p></td></tr><tr><td style="background:#fff;padding:32px 40px;"><p style="margin:0 0 6px;color:#374151;font-size:16px;font-weight:600;">Hi {{user_name}},</p><p style="margin:0 0 24px;color:#6b7280;font-size:14px;line-height:1.7;">Your support ticket has been received. Our team will review it and get back to you shortly.</p><table role="presentation" style="width:100%;border-collapse:collapse;border-radius:8px;overflow:hidden;margin-bottom:24px;"><tr><td style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:20px;"><table style="width:100%;border-collapse:collapse;"><tr><td style="padding:0 0 12px;border-bottom:1px solid #e2e8f0;"><div style="color:#9ca3af;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;margin-bottom:4px;">Ticket ID</div><div style="color:#00c6ff;font-size:20px;font-weight:800;">#{{ticket_id}}</div></td></tr><tr><td style="padding:12px 0 0;"><div style="color:#9ca3af;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;margin-bottom:4px;">Subject</div><div style="color:#1f2937;font-size:14px;font-weight:600;">{{subject}}</div></td></tr></table></td></tr></table><table role="presentation" style="width:100%;border-collapse:collapse;"><tr><td style="text-align:center;padding:4px 0 8px;"><a href="{{ticket_url}}" style="display:inline-block;padding:13px 36px;background:linear-gradient(135deg,#00c6ff,#a855f7);border-radius:8px;color:#fff;font-size:15px;font-weight:700;text-decoration:none;letter-spacing:.02em;">View Your Ticket</a></td></tr></table></td></tr><tr><td style="background:#f8fafc;padding:18px 40px;text-align:center;border-top:1px solid #e2e8f0;"><p style="margin:0;color:#9ca3af;font-size:12px;line-height:1.6;">You received this email because you opened a support ticket.<br>If this wasn\'t you, please ignore this message.</p></td></tr></table></td></tr></table></body></html>';
                $db->query(
                    "UPDATE mail_notification_templates SET body = ?, variables = '[\"ticket_id\",\"subject\",\"user_name\",\"ticket_url\",\"description\",\"app_name\"]' WHERE slug = 'support-ticket-created'",
                    [$bodyCreated]
                );
            }

            $stReply = $db->fetch(
                "SELECT id, body FROM mail_notification_templates WHERE slug = 'support-ticket-reply' LIMIT 1",
                []
            );
            if ($stReply && strpos($stReply['body'] ?? '', '{{app_name}}') === false) {
                $bodyReply = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="margin:0;padding:0;background:#f0f4f8;font-family:\'Segoe UI\',Arial,sans-serif;"><table role="presentation" style="width:100%;border-collapse:collapse;"><tr><td style="padding:32px 16px;"><table role="presentation" style="max-width:600px;margin:0 auto;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.10);"><tr><td style="background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%);padding:36px 40px 28px;text-align:center;"><div style="display:inline-block;width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,#f59e0b,#ef4444);text-align:center;line-height:52px;font-size:24px;margin-bottom:14px;">💬</div><h1 style="margin:0;color:#fff;font-size:22px;font-weight:700;letter-spacing:-.3px;">New Reply on Ticket #{{ticket_id}}</h1><p style="margin:8px 0 0;color:rgba(255,255,255,.65);font-size:13px;">{{app_name}} Support</p></td></tr><tr><td style="background:#fff;padding:32px 40px;"><p style="margin:0 0 6px;color:#374151;font-size:16px;font-weight:600;">Hi {{user_name}},</p><p style="margin:0 0 20px;color:#6b7280;font-size:14px;line-height:1.7;">A support agent has replied to your ticket. See the message below.</p><div style="margin-bottom:16px;"><div style="color:#9ca3af;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px;">Subject</div><div style="color:#1f2937;font-size:14px;font-weight:600;padding:10px 14px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;">{{subject}}</div></div><div style="margin-bottom:24px;"><div style="color:#9ca3af;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px;">Agent Reply</div><div style="padding:16px 18px;background:#fffbeb;border:1px solid #fde68a;border-left:4px solid #f59e0b;border-radius:0 8px 8px 0;color:#374151;font-size:14px;line-height:1.7;">{{reply_message}}</div></div><table role="presentation" style="width:100%;border-collapse:collapse;"><tr><td style="text-align:center;padding:4px 0 8px;"><a href="{{ticket_url}}" style="display:inline-block;padding:13px 36px;background:linear-gradient(135deg,#f59e0b,#ef4444);border-radius:8px;color:#fff;font-size:15px;font-weight:700;text-decoration:none;">View &amp; Reply</a></td></tr></table></td></tr><tr><td style="background:#f8fafc;padding:18px 40px;text-align:center;border-top:1px solid #e2e8f0;"><p style="margin:0;color:#9ca3af;font-size:12px;line-height:1.6;">You received this because there is an update on your support ticket.<br>Log in to view the full thread and reply.</p></td></tr></table></td></tr></table></body></html>';
                $db->query(
                    "UPDATE mail_notification_templates SET body = ?, variables = '[\"ticket_id\",\"subject\",\"user_name\",\"ticket_url\",\"reply_message\",\"status\",\"app_name\"]' WHERE slug = 'support-ticket-reply'",
                    [$bodyReply]
                );
            }

            $stStatus = $db->fetch(
                "SELECT id, body FROM mail_notification_templates WHERE slug = 'support-ticket-status-update' LIMIT 1",
                []
            );
            if ($stStatus && strpos($stStatus['body'] ?? '', '{{app_name}}') === false) {
                $bodyStatus = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="margin:0;padding:0;background:#f0f4f8;font-family:\'Segoe UI\',Arial,sans-serif;"><table role="presentation" style="width:100%;border-collapse:collapse;"><tr><td style="padding:32px 16px;"><table role="presentation" style="max-width:600px;margin:0 auto;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.10);"><tr><td style="background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%);padding:36px 40px 28px;text-align:center;"><div style="display:inline-block;width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,#10b981,#3b82f6);text-align:center;line-height:52px;font-size:24px;margin-bottom:14px;">🔔</div><h1 style="margin:0;color:#fff;font-size:22px;font-weight:700;letter-spacing:-.3px;">Ticket #{{ticket_id}} Status Updated</h1><p style="margin:8px 0 0;color:rgba(255,255,255,.65);font-size:13px;">{{app_name}} Support</p></td></tr><tr><td style="background:#fff;padding:32px 40px;"><p style="margin:0 0 6px;color:#374151;font-size:16px;font-weight:600;">Hi {{user_name}},</p><p style="margin:0 0 20px;color:#6b7280;font-size:14px;line-height:1.7;">The status of your support ticket has been updated by our team.</p><table role="presentation" style="width:100%;border-collapse:collapse;border-radius:8px;overflow:hidden;margin-bottom:20px;"><tr><td style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:20px;"><table style="width:100%;border-collapse:collapse;"><tr><td style="padding:0 0 12px;border-bottom:1px solid #e2e8f0;"><div style="color:#9ca3af;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;margin-bottom:4px;">Ticket</div><div style="color:#1f2937;font-size:14px;font-weight:600;">#{{ticket_id}} — {{subject}}</div></td></tr><tr><td style="padding:12px 0 0;"><div style="color:#9ca3af;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px;">New Status</div><span style="display:inline-block;padding:4px 14px;background:#dcfce7;border:1px solid #86efac;border-radius:20px;color:#15803d;font-size:13px;font-weight:700;">{{status}}</span></td></tr></table></td></tr></table><div style="margin-bottom:24px;"><div style="color:#9ca3af;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px;">Note from Support Team</div><div style="padding:16px 18px;background:#eff6ff;border:1px solid #bfdbfe;border-left:4px solid #3b82f6;border-radius:0 8px 8px 0;color:#374151;font-size:14px;line-height:1.7;">{{note}}</div></div><table role="presentation" style="width:100%;border-collapse:collapse;"><tr><td style="text-align:center;padding:4px 0 8px;"><a href="{{ticket_url}}" style="display:inline-block;padding:13px 36px;background:linear-gradient(135deg,#10b981,#3b82f6);border-radius:8px;color:#fff;font-size:15px;font-weight:700;text-decoration:none;">View Your Ticket</a></td></tr></table></td></tr><tr><td style="background:#f8fafc;padding:18px 40px;text-align:center;border-top:1px solid #e2e8f0;"><p style="margin:0;color:#9ca3af;font-size:12px;line-height:1.6;">You received this because the status of your support ticket was updated.</p></td></tr></table></td></tr></table></body></html>';
                $db->query(
                    "UPDATE mail_notification_templates SET body = ?, variables = '[\"ticket_id\",\"subject\",\"user_name\",\"ticket_url\",\"status\",\"note\",\"app_name\"]' WHERE slug = 'support-ticket-status-update'",
                    [$bodyStatus]
                );
            }
        } catch (\Throwable $e) {

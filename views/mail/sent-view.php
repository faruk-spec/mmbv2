...
<!-- ── Reply thread ──────────────────────────────────────────────────── -->
<?php if (!empty($thread)): ?>
<div style="margin-bottom:8px;font-size:12px;color:#64748b;text-transform:uppercase;letter-spacing:.5px;font-weight:600;padding-left:4px;">
    <i class="fas fa-code-branch"></i> Conversation thread (<?= count($thread) ?> message<?= count($thread) === 1 ? '' : 's' ?>)
</div>
<?php foreach ($thread as $t): ?>
<div class="mail-card" style="border-left:3px solid #667eea;opacity:.85;">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;flex-wrap:wrap;gap:8px;">
        <div>
            <div style="font-size:13px;font-weight:600;color:#e2e8f0;">
                <?= htmlspecialchars($t['subject'] ?? '(no subject)', ENT_QUOTES, 'UTF-8') ?>
            </div>
        ...
        <div style="display:flex;align-items:center;gap:10px;">
            <?php if (($t['status'] ?? '') === 'sent'): ?>
            <span style="color:#6ee7b7;font-size:12px;"><i class="fas fa-check-circle"></i></span>
            <?php else: ?>
            <span style="color:#fca5a5;font-size:12px;"><i class="fas fa-exclamation-circle"></i></span>
            <?php endif; ?>
            <span style="font-size:12px;color:#64748b;"><?= !empty($t['sent_at']) ? date('M j, g:i a', strtotime($t['sent_at'])) : '' ?></span>
            <a href="/mail/sent/view/<?= (int)$t['id'] ?>" class="btn btn-sm btn-secondary" style="font-size:11px;padding:4px 8px;">View</a>
        </div>
    </div>
    <?php if (!empty($t['body_html'])): ?>
    <div style="background:#fff;border-radius:6px;padding:14px;color:#222;font-size:13px;line-height:1.6;overflow-x:auto;max-height:200px;overflow-y:auto;">
        <?= $t['body_html'] ?>
    </div>
    <?php elseif (!empty($t['body_text'])): ?>
    <div style="background:#0d0d14;border-radius:6px;padding:14px;color:#94a3b8;font-size:12px;line-height:1.6;overflow-x:auto;max-height:200px;overflow-y:auto;white-space:pre-wrap;font-family:monospace;">
        <?= htmlspecialchars(mb_substr($t['body_text'], 0, 500), ENT_QUOTES, 'UTF-8') ?><?= strlen($t['body_text'] ?? '') > 500 ? '…' : '' ?>
    </div>
    <?php endif; ?>
</div>
<?php endforeach; ?>

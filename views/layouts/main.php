...
window.mmbSkeleton = (function(){
    ...
    <div id="new-login-alert-banner" style="position:fixed;top:<?= !empty($_SESSION['_concurrent_session_warning']) ? '56px' : '0' ?>;left:0;right:0;z-index:99998;background:linear-gradient(135deg,#9945ff,#0099cc);color:#fff;padding:12px 20px;display:flex;align-items:center;justify-content:space-between;box-shadow:0 4px 20px rgba(0,0,0,0.4);font-family:'Poppins',sans-serif;font-size:14px;">
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <span>⚠️ New login detected on your account from <?php if (!empty($nlAlert['device'])): ?><strong><?= htmlspecialchars($nlAlert['device'], ENT_QUOTES) ?></strong> (<?php endif; ?>IP <strong><?= htmlspecialchars($nlAlert['ip'] ?? 'unknown', ENT_QUOTES) ?></strong><?php if (!empty($nlAlert['device'])): ?>)<?php endif; ?>. Not you? <a href="/security" style="color:#fff;text-decoration:underline;">Review sessions</a></span>
        </div>
    ...
    (function () {
        function showNewLoginBanner(ip, device) {
            ...
            var deviceInfo = device ? '<strong>' + device + '</strong> (' : '';
            var deviceClose = device ? ')' : '';
            banner.innerHTML = '<div style="display:flex;align-items:center;gap:10px;"><i class="fas fa-sign-in-alt"></i><span>⚠️ New login detected on your account from ' + deviceInfo + 'IP <strong>' + (ip || 'unknown') + '</strong>' + deviceClose + '. Not you? <a href="/security" style="color:#fff;text-decoration:underline;">Review sessions</a></span></div>'
                + '<button onclick="this.parentElement.style.display=\'none\'" style="background:rgba(0,0,0,0.2);border:1px solid rgba(255,255,255,0.4);color:#fff;padding:5px 12px;border-radius:6px;cursor:pointer;font-size:13px;font-family:inherit;">✕</button>';
        ...
        function poll() {
            ...
            fetch('/api/session-alerts', { credentials: 'same-origin' })
                ...
                .then(function(d) { if (d && d.alert) showNewLoginBanner(d.alert.ip, d.alert.device); })
                .catch(function() {});
        }
    ...
    $footerShowSocial = '1';
    $footerSocialLinks = [];
    // Homepage 3-column footer settings
    $hpFooterEnabled  = '0';
    $hpFooterSettings = [];
    try {
        $footerRows = $db->fetchAll(
            "SELECT `key`, value FROM settings WHERE `key` LIKE 'footer_%' OR `key` LIKE 'hp_footer_%'"
        );
        foreach ($footerRows as $fr) {
            $fk = $fr['key'];
            if ($fk === 'footer_tagline')        $footerTagline   = $fr['value'] ?? '';
            elseif ($fk === 'footer_copyright')  $footerCopyright = $fr['value'] ?? '';
            elseif ($fk === 'footer_show_social') $footerShowSocial = $fr['value'] ?? '1';
            elseif (in_array($fk, ['footer_social_twitter','footer_social_github','footer_social_linkedin','footer_social_youtube'], true)) {
                if (!empty($fr['value'])) $footerSocialLinks[$fk] = $fr['value'];
            } elseif (strpos($fk, 'hp_footer_') === 0) {
        ...
        <div class="container" style="max-width:1200px;margin:0 auto;padding:0 24px;">
            <div class="site-footer-grid">
                ...
                <div>
                    <div style="font-weight:800;font-size:.95rem;color:var(--text-primary);margin-bottom:12px;letter-spacing:.02em;">
                        <?= htmlspecialchars($hpFooterSettings['hp_footer_col1_heading'] ?? 'About Us') ?>
                    </div>
                    <p style="color:var(--text-secondary);font-size:.84rem;line-height:1.7;">
                        <?= nl2br(htmlspecialchars($hpFooterSettings['hp_footer_col1_text'] ?? '')) ?>
                    </p>
                </div>
                <!-- Column 2 — Quick Links (from pages with show_footer=1) -->
                <div>
                    <div style="font-weight:800;font-size:.95rem;color:var(--text-primary);margin-bottom:12px;letter-spacing:.02em;">
                ...
                <div>
                    ...
                    <div style="display:flex;gap:14px;align-items:center;margin-top:12px;">
                        ...
                        <a href="<?= htmlspecialchars($sv) ?>" target="_blank" rel="noopener" style="color:var(--text-secondary);transition:color .2s;" onmouseover="this.style.color='var(--cyan)'" onmouseout="this.style.color='var(--text-secondary)'">
                            <?php if ($sk === 'footer_social_twitter'): ?><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.748l7.73-8.835L1.254 2.25H8.08l4.261 5.638L18.244 2.25z"/></svg>
                            <?php elseif ($sk === 'footer_social_github'): ?><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"/></svg>
                            <?php elseif ($sk === 'footer_social_linkedin'): ?><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                            <?php elseif ($sk === 'footer_social_youtube'): ?><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg><?php endif; ?>
                        </a>
                        <?php endforeach; ?>
                    </div>
                    <?php endif; ?>
            ...
            <div style="border-top:1px solid var(--border-color);padding:16px 0;text-align:center;">
                <p style="font-size:.8rem;color:var(--text-secondary);">
                    <?php
                    $hpBottom = $hpFooterSettings['hp_footer_bottom_text'] ?? '';
                    if (!empty($hpBottom)) {
                        echo htmlspecialchars($hpBottom);
                    } elseif (!empty($footerCopyright)) {

...
<?php if (!empty($enabledProviders)): ?>
    <?php foreach ($enabledProviders as $provider): ?>
        ...
        <?php if (!isset($buttonTextMap[$oauthMode][$providerName])) { continue; } ?>
        <div style="margin-bottom: 8px;">
            <a href="/auth/<?= View::e($providerName) ?><?= $oauthMode === 'register' ? '?register=1' : '' ?>" class="btn" style="width: 100%; background: #fff; color: #333; border: 1px solid #ddd; font-size: 0.95rem; padding: 12px 20px;">
                ...
                <?php elseif ($providerName === 'github'): ?>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="#111827" xmlns="http://www.w3.org/2000/svg" style="flex-shrink:0;">

/**
 * CardX ID Card — Shared render functions.
 * Used by both the standard generate page and the AI generate page.
 *
 * Pure functions only — no DOM reads or writes.
 *
 * Globals: ALL_STYLES, STYLE_COLOR_PRESETS, FIELD_SHORT
 * Functions: isPortraitStyle, fieldRowsHTML, qrSlotHTML,
 *            buildStyleThumbnail, renderCard, renderClassic…renderVBadge
 *
 * v object shape expected by renderXxx(v):
 *   { pri, acc, bg, txt, font, nameVal, roleVal, orgVal, addrVal,
 *     fieldItems:[{label,val}], photoHTML, photoShapeCSS, tplName, portrait }
 */
var ALL_STYLES = [
...
//  Dispatcher
// =============================================================================
function renderCard(style, v) {
    switch (style) {
        case 'sidebar':       return renderSidebar(v);
        case 'wave':          return renderWave(v);
        case 'bold_header':   return renderBoldHeader(v);
        case 'diagonal':      return renderDiagonal(v);
        case 'gradient_pro':  return renderGradientPro(v);
        case 'neon':          return renderNeon(v);
        case 'executive':     return renderExecutive(v);
        case 'stripe':        return renderStripe(v);
        case 'metro':         return renderMetro(v);
        case 'glass':         return renderGlass(v);
        case 'zigzag':        return renderZigzag(v);
        case 'ribbon':        return renderRibbon(v);
        case 'v_sharp':       return renderVSharp(v);
        case 'v_curve':       return renderVCurve(v);
        case 'v_hex':         return renderVHex(v);
        case 'v_circle':      return renderVCircle(v);
        case 'v_split':       return renderVSplit(v);
        case 'v_ribbon':      return renderVRibbon(v);
        case 'v_arch':        return renderVArch(v);
        case 'v_diamond':     return renderVDiamond(v);
        case 'v_corner':      return renderVCorner(v);
        case 'v_dual':        return renderVDual(v);
        case 'v_stripe':      return renderVStripe(v);
        case 'v_badge':       return renderVBadge(v);
        default:              return renderClassic(v);
    }
}

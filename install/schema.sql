...
CREATE TABLE IF NOT EXISTS `home_content` (
    ...
    `global_thumb_intensity`   TINYINT UNSIGNED NOT NULL DEFAULT 60 COMMENT 'Universal thumbnail opacity 0-100',
    `override_thumb_intensity` TINYINT(1)       NOT NULL DEFAULT 0  COMMENT '1 = apply global intensity to all project cards',
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `home_projects` (
    ...
    `show_title`      TINYINT(1)       NOT NULL DEFAULT 1  COMMENT '1 = show project title on card, 0 = hide',
    `thumb_intensity` TINYINT UNSIGNED NOT NULL DEFAULT 60 COMMENT 'Thumbnail image opacity 0-100',
